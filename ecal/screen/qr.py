...
def add_qr_code(url, surface, image):
    ...
    px = int((half_sw - qr_background_w) / 2) + half_sw
    py = int((s_h - qr_background_h) / 2)

    if px < 0 or py < 0:
        raise QRCodeError(
            f"Oh oh, image is {qr_background_w}x{qr_background_h} and doesn't fit surface {surface}!"
        )

    image.paste(qr_background, (px, py, px + qr_background_w, py + qr_background_h))
    return image
